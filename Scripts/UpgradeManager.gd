...
func buy_item(item_id: String) -> bool:
	...
	var current_level: int = File.data["player"]["items_owned"].get(item_id, 0)
	...
	if File.data["player"]["coins"] < cost:
		return false

	File.data["player"]["coins"] -= cost
	File.data["player"]["items_owned"][item_id] = current_level + 1
...
func apply_items() -> void:
	...
	for item_id in File.data["player"]["items_owned"].keys():
		var item = DataManager.items_db.get(item_id)
		if item == null:
			continue

		var owned_level: int = File.data["player"]["items_owned"][item_id]
...
func get_item_level(item_id: String) -> int:
	return File.data["player"]["items_owned"].get(item_id, 0)
